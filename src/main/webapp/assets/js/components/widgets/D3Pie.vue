...
<script>
  module.exports = {
      ...
      computed: {
          ...
          data() {
            if (this.debate.comments) {
                  ...
                  data.forEach((d, i) => {
                      ...
                      if (this.byOpinion) {
                        d.color = (d.key === 'FOR' ? blueColor : d.key === 'AGAINST' ? redColor : d.key === 'NEUTRAL' ? greyColor : '')
                      } else {
                        d.color = this.colors ? this.colors.get(d.values[0].owner) : d3.interpolateRainbow(i / data.length);
                      }
                      d.letterCount = d.values.reduce((acc, c) => acc + c.argumentation.length, 0);
                      this.letterCount += d.letterCount;
                      d.commentCount = d.values.length;
                      this.commentCount += d.commentCount;
                  });

                  if (this.byLetter) {
                      data.sort((a, b) => a.letterCount - b.letterCount);
                  } else {
                      data.sort((a, b) => a.commentCount - b.commentCount);
                  }
                  return data;
              }
              return [];
          ...
          slices() {
              const slices = this.pie(this.data);

<template>
  <modal-layout v-bind:movable="true">
      ...
      <template v-if="comment.opinion">
          <text-input
              v-if="comment.debate.reformulationMandatory && comment.opinion !== 'NEUTRAL'"
...
<script>
  module.exports = {
      ...
      computed: {
          isFilled() {
              return this.comment.opinion 
                && this.comment.argumentation 
                && (!this.comment.debate.reformulationMandatory
                || (this.comment.opinion === 'NEUTRAL' || this.comment?.reformulation?.trim().length > 0));
          }
      },
      watch: {
